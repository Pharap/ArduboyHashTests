# Statistics

## Compilation Size

The first size is the 'print test', `arduboy.println(static_cast<int>(Hash::hash(object)))`, the second size is the 'store test', `EEPROM::putWithHash<Hash>(0, object)`.

### SummationHash

#### int32_t

```text
Sketch uses 6706 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1199 bytes (46%) of dynamic memory, leaving 1361 bytes for local variables. Maximum is 2560 bytes.
```

```text
Sketch uses 6580 bytes (22%) of program storage space. Maximum is 28672 bytes.
Global variables use 1197 bytes (46%) of dynamic memory, leaving 1363 bytes for local variables. Maximum is 2560 bytes.
```

#### uint32_t

```text
Sketch uses 6706 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1199 bytes (46%) of dynamic memory, leaving 1361 bytes for local variables. Maximum is 2560 bytes.
```

```text
Sketch uses 6580 bytes (22%) of program storage space. Maximum is 28672 bytes.
Global variables use 1197 bytes (46%) of dynamic memory, leaving 1363 bytes for local variables. Maximum is 2560 bytes.
```

### CSharpTupleHash

#### int32_t

```text
Sketch uses 6770 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1199 bytes (46%) of dynamic memory, leaving 1361 bytes for local variables. Maximum is 2560 bytes.
```

```text
Sketch uses 6670 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1197 bytes (46%) of dynamic memory, leaving 1363 bytes for local variables. Maximum is 2560 bytes.
```

#### uint32_t

```text
Sketch uses 6770 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1199 bytes (46%) of dynamic memory, leaving 1361 bytes for local variables. Maximum is 2560 bytes.
```

```text
Sketch uses 6670 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1197 bytes (46%) of dynamic memory, leaving 1363 bytes for local variables. Maximum is 2560 bytes.
```

### Adler32

```text
Sketch uses 6792 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1199 bytes (46%) of dynamic memory, leaving 1361 bytes for local variables. Maximum is 2560 bytes.
```

```text
Sketch uses 6720 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1197 bytes (46%) of dynamic memory, leaving 1363 bytes for local variables. Maximum is 2560 bytes.
```

### FNV32

```text
Sketch uses 6808 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1199 bytes (46%) of dynamic memory, leaving 1361 bytes for local variables. Maximum is 2560 bytes.
```

```text
Sketch uses 6708 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1197 bytes (46%) of dynamic memory, leaving 1363 bytes for local variables. Maximum is 2560 bytes.
```

### FNV32a

```text
Sketch uses 6808 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1199 bytes (46%) of dynamic memory, leaving 1361 bytes for local variables. Maximum is 2560 bytes.
```

```text
Sketch uses 6708 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1197 bytes (46%) of dynamic memory, leaving 1363 bytes for local variables. Maximum is 2560 bytes.
```

### KnuthHash

```text
Sketch uses 6742 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1199 bytes (46%) of dynamic memory, leaving 1361 bytes for local variables. Maximum is 2560 bytes.
```

```text
Sketch uses 6616 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1197 bytes (46%) of dynamic memory, leaving 1363 bytes for local variables. Maximum is 2560 bytes.
```

### OneAtATimeHash

```text
Sketch uses 6834 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1199 bytes (46%) of dynamic memory, leaving 1361 bytes for local variables. Maximum is 2560 bytes.
```

```text
Sketch uses 6708 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1197 bytes (46%) of dynamic memory, leaving 1363 bytes for local variables. Maximum is 2560 bytes.
```

### PartialOneAtATimeHash

```text
Sketch uses 6796 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1199 bytes (46%) of dynamic memory, leaving 1361 bytes for local variables. Maximum is 2560 bytes.
```

```text
Sketch uses 6666 bytes (23%) of program storage space. Maximum is 28672 bytes.
Global variables use 1197 bytes (46%) of dynamic memory, leaving 1363 bytes for local variables. Maximum is 2560 bytes.
```

### ZeroHash

```text
Sketch uses 6476 bytes (22%) of program storage space. Maximum is 28672 bytes.
Global variables use 1199 bytes (46%) of dynamic memory, leaving 1361 bytes for local variables. Maximum is 2560 bytes.
```

```text
Sketch uses 6582 bytes (22%) of program storage space. Maximum is 28672 bytes.
Global variables use 1197 bytes (46%) of dynamic memory, leaving 1363 bytes for local variables. Maximum is 2560 bytes.
```